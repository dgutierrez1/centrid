...
export function ReferencePill({
  ...
  const pill = (
    <button
      onClick={onClick}
      className={`group relative flex items-center gap-2 px-3 py-2 bg-primary-100 dark:bg-primary-800/50 hover:bg-primary-200 dark:hover:bg-primary-700/50 rounded-full transition-colors border border-primary-200 dark:border-primary-700 shadow-sm shrink-0 ${className}`}
    >
      {/* Icon */}
      {iconMap[referenceType]}

      {/* File name (truncated) - matches primary color scheme */}
      <span className="text-sm font-medium text-primary-700 dark:text-primary-300 truncate max-w-[100px]">
        {name}
      </span>
