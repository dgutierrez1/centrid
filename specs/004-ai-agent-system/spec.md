...
**What we're building**: An exploration workspace where users can branch threads to explore multiple approaches in parallel, capture findings as persistent files with provenance, and consolidate insights from the entire exploration tree.
...
**Our solution**: Branching threads + persistent filesystem + provenance tracking + cross-branch context = complete exploration workflow (Explore → Capture → Reference → Consolidate → Ship)
...
**Core insight**: Files are artifacts of exploration. Threads are exploration paths. Provenance connects them. Don't force users to choose between "file view" or "thread view" - they're the same thing from different angles.
...
- Q: How should thread summaries be generated when "auto-updated on every message" - incremental update or full regeneration? → A: Regenerate summary and embedding from scratch on every new message (not incremental updates)
...
- Q: What information should thread summaries include (topics, decisions, artifacts, questions)? → A: Topics + key decisions + artifacts created + open questions (comprehensive context for consolidation and cross-branch discovery)
...
- Q: Should memory chunk summaries be generated from just the 10 messages in that chunk or all messages up to that point? → A: From the 10 messages in that chunk only (localized summaries for better semantic retrieval of specific thread segments)
...
- Q: When user @-mentions Thread A which @-mentions Thread B which @-mentions Thread C, how to prevent circular reference and infinite context expansion? → A: No nested resolution (depth 1 only) - load @-mentioned thread's direct content (summary + last 5 messages + explicit files + artifacts) WITHOUT expanding any @-mentions inside it (prevents infinite loops, keeps context predictable, simple for MVP)
...
- Q: Can users @-mention other threads to add them to context? → A: Yes, users should be able to @-mention other threads to add their context (files, messages, artifacts)
...
- Q: Should context panel be separate UI component or integrated into thread? → A: Context section integrated into thread (above input box, before messages) showing different context types with visual priority indicators
...
- Q: When user @-mentions another thread, what content should be included in context? → A: Thread summary + last 5 messages + all explicit files + all artifacts from @-mentioned thread
- Q: How should nested branches inherit context (Main → A → A1)? → A: Inherit explicit files from immediate parent only + parent thread summary + parent's last message + branching user message content (if agent-created branch)
...
- Q: Should all domain entities (files, threads, knowledge graph nodes) have embeddings and summaries in a unified layer? → A: Yes - All entities get embeddings + summary + structure metadata in unified shadow layer
...
- Q: How often should thread embeddings and summaries be updated? → A: Trigger embedding and summary generation on every message (not just every 10 messages) for maximum context freshness
...
- Q: When should prime context be built for agent requests? → A: Build prime context BEFORE sending request to agent, assembling most relevant and optimal data across all domains (files, threads, KG) so agent can perform actions optimally and know which tools to call
...
- Q: Should context management use dynamic priming only, memory chunking only, or hybrid? → A: Hybrid - Dynamic priming optimizes per-request context (show excluded items in UI for manual re-priming), memory chunking compresses old thread messages into embeddings when thread exceeds manageable size (retrieve relevant chunks semantically per request)
...
- Q: How should desktop workspace layout balance thread-first UX with file editing needs? → A: Adaptive 3-panel layout - Left panel (20%, tabs for Files/Threads, defaults to Threads on open, no persistence), Center panel (40-80% thread, expands when no file open), Right panel (0-40% editor, appears only when file opened, only one file at a time, close button to dismiss)
...
### User Story 1 - Branch Threads for Parallel Exploration (Priority: P1)

A user exploring a complex topic wants to try multiple approaches simultaneously without losing context between threads. They need to branch their thread to explore different angles (e.g., "RAG approach" vs "Fine-tuning approach") while maintaining the original exploration context.
...
3. **Given** user has 3 branches (Main → RAG, Main → Orchestration), **When** user switches between branches, **Then** each branch maintains separate thread history while sharing explicitly added files from parent
...
A user wants AI to create files (analysis documents, summaries, decision docs) from thread insights and have those files automatically track which thread created them, when, and why. Files should be persistent and reusable across all threads.
...
5. **Given** agent creates file "rag-analysis.md" in thread, **When** next message is sent in same thread, **Then** file is auto-included in explicit context (agent remembers what it just created)
...
4. **Given** user approves consolidated document creation, **When** file is created, **Then** system stores provenance with multiple source threads (created from Main, references Branch A + Branch B + Branch C)
...
A user wants to understand where files came from, which thread created them, and navigate back to the source thread to see the full context that led to artifact creation. This builds trust and enables exploration traceability.
...
1. **Given** user views file "rag-analysis.md" in context references, **When** they click provenance indicator, **Then** system shows modal with: source branch name, creation timestamp, context summary (2-3 sentences about what the thread was exploring), and "Go to source thread" link
2. **Given** user clicks "Go to source thread", **When** navigation happens, **Then** system opens source branch at the exact message where file was created, with highlight on the creation event
3. **Given** user is in thread viewing context panel, **When** they hover over file in "Semantic matches" section, **Then** tooltip shows: "From: RAG Deep Dive branch (sibling, +0.15 weight), Created: 2 hours ago, Relevance: 0.87"
4. **Given** user has Visual Tree View enabled (Phase 3), **When** they click on file node in tree, **Then** system highlights all threads that referenced this file (provenance graph visualization)
...
#### Branching & Thread Graph Management

- **FR-001**: System MUST support creating child branches from any thread with parent-child relationship (DAG structure)
- **FR-002**: System MUST inherit from immediate parent when child branch is created: explicit files (context references) + parent thread summary + parent's last message + branching message content (if agent-created the branch via `create_branch`)
...
- **FR-004**: System MUST support user-initiated branching via "Create Branch" button in thread header
- **FR-005**: System MUST support agent-initiated branching via `create_branch(title, context_files)` tool call with user approval
- **FR-006**: System MUST support system-prompted branching when topic shift is detected (semantic distance >0.7 from parent thread)
...
- **FR-009a**: System MUST generate and maintain thread summary for each thread using embeddings/summarization (regenerate summary and embedding from scratch on every message for maximum context freshness, not incremental updates). Summary MUST include: topics discussed, key decisions made, artifacts created, and open questions
...
- **FR-010a**: System MUST maintain a separate `shadow_entities` table as the unified shadow domain for all searchable entities (files, threads, knowledge graph nodes)
- **FR-010b**: System MUST create shadow entity entry for each domain entity containing: entity_id (reference to source entity), entity_type (file/thread/kg_node), embedding (768-dim vector), summary (auto-generated 2-3 sentence description), structure_metadata (JSONB entity-specific structure: for files = {outline: headings hierarchy array, key_concepts: string array, main_topics: string array}, for threads = {topics: string array}, for kg_nodes = {relationships: object})
...
- **FR-010d**: System MUST support unified semantic search across all entity types via single query to shadow_entities table ("find files OR threads OR concepts about RAG")
- **FR-010e**: System MUST update shadow entity summary when source entity changes significantly (files: >20% character diff from last snapshot triggers regeneration of embedding + summary + structure_metadata, threads: regenerate from scratch on every message, KG nodes: relationship changes)
...
- **FR-015**: System MUST auto-include newly created files in explicit context for next message in same thread (agent remembers what it created)
...
- **FR-017**: System MUST display provenance indicators in file UI only for agent-created files (pills, editor header, file browser) showing source thread
...
- **FR-019**: System MUST navigate to source thread at exact message where file was created when user clicks "Go to source"
...
- **FR-021**: System MUST perform semantic search across shadow_entities table to find relevant files, threads, and concepts (not limited to current branch) when building context
...
- **FR-021b**: System MUST support entity type filtering in semantic search (e.g., "files only", "threads only", "all types") based on context assembly needs
...
- **FR-024**: System MUST display context section integrated into thread (above input box, before messages) with collapsible context type groups: (1) Explicit context, (2) Semantic matches, (3) Branch context, (4) Artifacts from this thread, (5) Knowledge graph connections (background), (6) Excluded from context (items that didn't fit in 200K budget, user can manually re-prime)
...
- **FR-024b**: System MUST build prime context BEFORE sending request to agent, automatically assembling most relevant and optimal data across all domains (files via shadow entities, threads, KG nodes) so agent can perform actions optimally and determine which tools to call
...
- **FR-024e**: System MUST implement memory chunking for threads exceeding 40 messages: compress messages 1-30 into embedding chunks (stored in shadow_entities or conversation_memory table), keep messages 31-40 as full text, retrieve relevant memory chunks semantically per request
- **FR-024f**: System MUST chunk thread history in batches of 10 messages (messages 1-10 → chunk 1, messages 11-20 → chunk 2, etc.) with each chunk containing: embedding, summary (generated from the 10 messages in that chunk only, not cumulative), message_ids array, timestamp_range
- **FR-024g**: System MUST retrieve top 3 most relevant memory chunks when building context for long threads (>40 messages), combining: latest 10 full messages + 3 relevant memory chunks (summarized) + files + branch context
...
- **FR-026**: System MUST support @-mention autocomplete for both files and other threads with indicators in dropdown ("rag-analysis.md [RAG Deep Dive branch]", "Thread: RAG vs Fine-tuning [Main branch]")
- **FR-026a**: System MUST allow users to @-mention other threads to add their context to current thread: thread summary (from embeddings/summarization) + last 5 messages + all explicit files + all artifacts from @-mentioned thread. System MUST NOT expand nested @-mentions (depth 1 only): if @-mentioned thread contains @-mentions to other threads, those are ignored during resolution to prevent circular references and infinite context expansion
- **FR-027**: System MUST prioritize context within 200K token limit using hybrid approach: Explicit references (1.0 weight, never truncated) > Semantic matches (0.5 base + modifiers) > Branch context (0.7 base) > Thread history (latest 10 full messages + top 3 relevant memory chunks if >40 messages, rest excluded but shown in UI)
- **FR-028**: REMOVED - Context budget NOT displayed to users (internal optimization only)
- **FR-029**: System MUST allow manual exclusion of sibling branches from context via "Hide from [Branch Name]" button in semantic match tooltip/dropdown. When clicked, system MUST blacklist that branch_id for current thread (store in conversation metadata), preventing future semantic matches from that branch. System MUST provide UI in context settings to view and remove blacklisted branches (per-thread scope)
...
- **FR-032**: System MUST store consolidated file provenance with multiple source threads (array of conversation_ids that contributed to consolidated document)
- **FR-033**: System MUST handle context window limits during consolidation using hybrid context management: prioritize high-relevance artifacts from all branches (semantic scoring + relationship modifiers), compress thread history from each branch using memory chunking (retrieve relevant chunks only), show excluded branches/artifacts in UI with option for user to manually include before consolidation proceeds
...
- **FR-040**: Visual tree view SHOULD show provenance graph: clicking file node highlights all threads that referenced it
...
- **FR-047a**: System MUST stream agent responses via Server-Sent Events (SSE) showing mixed content (text, tool calls, approval prompts) in execution order within same thread message
...
- **FR-053b**: System MUST broadcast thread updates (new messages, branch creation, file creation) to all connected client sessions via Supabase real-time subscriptions
...
- **FR-056**: System MUST enforce row-level security: users can only access their own threads, branches, files
...
- **Shadow Entity**: Unified semantic layer entry for all searchable domain entities - includes shadow_id, entity_id (reference to source file/thread/kg_node), entity_type (file/thread/kg_node), embedding (768-dim vector), summary (auto-generated 2-3 sentence description), structure_metadata (JSONB entity-specific: for files = {outline: headings hierarchy nested array, key_concepts: string array of extracted concepts, main_topics: string array of primary topics}, for threads = {topics: string array}, for kg_nodes = {relationships: object}), last_updated, owner_user_id

- **Thread (Branch)**: Represents a node in the exploration tree - includes conversation_id, parent_id (nullable for root/Main), branch_title, creation_timestamp, creator (user/agent/system), thread_summary (auto-generated from shadow entity summary, regenerated from scratch on every message, includes: topics discussed, key decisions made, artifacts created, open questions), messages array, explicit_files (inherited from parent + manually added), parent_last_message (cached from parent for child branch context), branching_message_content (if agent-created via `create_branch`), blacklisted_branches (array of conversation_ids manually excluded from semantic context via "Hide from [Branch Name]"), total_tokens_used, shadow_entity_id (reference to shadow_entities table)

- **Message**: Represents a single turn in thread - includes message_id, conversation_id, role (user/assistant), content, tool_calls array (for agent messages), timestamp, tokens_used

- **Thread Memory Chunk**: Compressed thread history for long threads (>40 messages) - includes chunk_id, conversation_id, message_ids array (messages compressed in this chunk), embedding (768-dim vector representing chunk content), summary (localized to this chunk only: topics + key decisions + artifacts created + open questions from the 10 messages in this chunk, not cumulative from all prior messages), timestamp_range (start_timestamp, end_timestamp), chunk_index (sequential: chunk 1 = messages 1-10, chunk 2 = messages 11-20, etc.), owner_user_id. Used for semantic retrieval when thread exceeds manageable size
...
- **Context Reference** (renamed from "Context Pill"): Represents explicit context in thread - includes reference_id, conversation_id, entity_type (file/folder/thread), entity_reference (file_path/folder_path/conversation_id), source (inherited/manual/agent-added/@-mentioned), display_label, added_timestamp, priority_tier (1=highest, 2=medium, 3=lower for visual grouping)
...
- **Knowledge Graph Edge** (Background Infrastructure): Represents AI-detected connections - includes edge_id, source_entity_id, source_type (file/thread/concept), target_entity_id, target_type, relationship_type (relates_to/used_in/evolved_from), confidence_score, creation_timestamp (NOTE: KG runs in background for future V2+ features, not exposed in MVP UI)
...
- Shadow entity queries: <300ms for unified search across all entity types (files + threads + kg_nodes)
...
- Thread summary update: <2s per message (background, asynchronous)
...
- Support 1000+ shadow entities per user across all entity types (files + threads + kg_nodes)
...
- Context section integrated into thread with visual priority indicators helps users understand what AI sees
- @-mention autocomplete for files and threads enables quick context addition
...
4. **Shadow Entity Infrastructure**: Unified `shadow_entities` table with entity_type discriminator for files, threads, and knowledge graph nodes
...
6. **Real-time Subscriptions**: Supabase real-time for broadcasting updates across devices (threads, files, shadow entities)
...
- Thread embeddings (searchable branches by semantic similarity)
- Advanced semantic search (thread history search, concept search)
...
1. Thread branching (parent-child DAG in database)
...
4. @-mention support for other threads (add thread context to current thread)
...
- Do users @-mention other threads to combine contexts? (>20% usage rate)
...
- @-mention autocomplete for threads appears <300ms
