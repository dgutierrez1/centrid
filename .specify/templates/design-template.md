...
<!--
  ...
  PROGRESS TRACKING: Mark completion status as designed components are built.
  RESPONSIVE VARIANTS: Document if mobile uses different component than desktop.
  CROSS-FEATURE REUSE: Document components reused from other features.
-->

| Screen Name | Component (Desktop) | Component (Mobile) | Location | Reused From | Priority | Screenshots |
|-------------|---------------------|-------------------|----------|-------------|----------|-------------|
| [Screen 1 Name] | `Screen1.tsx` | Same | `packages/ui/features/[feature]/` | - | P1 | ❌ |
| [Screen 2 Name] | `Screen2.tsx` | `Screen2Drawer.tsx` | `packages/ui/features/[feature]/` | - | P1 | ❌ |
| [Screen 3 Name] | `SharedCard.tsx` | Same | `packages/ui/features/other-feature/` | Feature A | P2 | ❌ |

**Legend**:
- Component (Mobile): "Same" if identical to desktop, or specify different component file
- Reused From: "-" if new to this feature, or specify originating feature
- Screenshots: ❌ Not generated | ✅ Generated
...
<!--
  OPTIONAL: Only fill if design intentionally deviates from ux.md specifications.
  Used by /speckit.verify-design to distinguish intentional changes from errors.
  Used by /speckit.tasks to preserve deviations during implementation.
-->

**IF design differs from ux.md** (layout dimensions, spacing, component props):

| Screen/Component | UX.md Spec | Actual Design | Rationale | Impact |
|------------------|------------|---------------|-----------|--------|
| [Name] | [e.g., "Right panel 30%"] | [e.g., "Right panel 25%"] | [e.g., "Better balance at 1440px width"] | [e.g., "Main area 75% vs 70%"] |
| [Name] | [e.g., "gap-4 (16px)"] | [e.g., "gap-3 (12px)"] | [e.g., "Tighter spacing for mobile"] | [e.g., "More items visible"] |

**IF no deviations**: Leave this section empty or write "No deviations from ux.md specifications"
...
<!--
  ...
  NEW REQUIREMENTS: If design phase reveals new requirements (not in ux.md/arch.md),
  mark them with ⚠️ NEW REQUIREMENT so /speckit.verify-design flags for review.
-->
...
- [e.g., "Streaming responses require SSE connection (see arch.md API section)"]
- [e.g., "Branch selector needs virtualization for 100+ branches (react-window)"]
- ⚠️ NEW REQUIREMENT: [e.g., "Conflict resolution modal needed for branch consolidation"]
